...
export default class InMemoryRepository implements WeatherRepository {
  ...
  private static generateMockWeather(): z.infer<typeof WeatherSchema> {
    const forecast = Array.from(
      ...
      (): z.infer<typeof ForecastSchema> => ({
        ...
        by_hour: Array.from({ length: 24 }, (_, i) => ({
          hour: new Date(`2021-01-01T${(i + 1).toString().padStart(2, '0')}:00:00`).toISOString(),
          temperature: generateRandomNumber(),
          humidity: generateRandomNumber(),
          wind_speed: generateRandomNumber(),
    ...
    return {
      ...
      average: {
        temperature: round(forecast.reduce((acc, { temperature }) => acc + temperature, 0) / forecast.length),
        humidity: round(forecast.reduce((acc, { humidity }) => acc + humidity, 0) / forecast.length),
        wind_speed: round(forecast.reduce((acc, { wind_speed }) => acc + wind_speed, 0) / forecast.length),
      },
    };
  }
